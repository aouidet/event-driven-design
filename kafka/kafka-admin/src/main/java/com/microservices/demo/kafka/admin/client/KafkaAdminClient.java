...
public class KafkaAdminClient {
    ...
    public void checkSchemaRegistry() {
        // create logic to simulate api schemaRegistry state
        int retryCount = 1;
    ...
    private CreateTopicsResult doCreateTopics(RetryContext retryContext) {
        ...
        LOG.info("Creating {} topic(s), attempts {}", topicsName.size(), retryContext.getRetryCount());
        List<NewTopic> kafkaTopics = topicsName.stream().map(topic -> new NewTopic(topic.trim(), kafkaConfigData.getNumOfPartitions(), kafkaConfigData.getReplicationFactor())).toList();
    ...
    private Collection<TopicListing> doGetTopics(RetryContext retryContext) throws ExecutionException, InterruptedException {
        LOG.info("Reading kafka topic {}, attempts {}", kafkaConfigData.getTopicNamesToCreate().toArray(),
